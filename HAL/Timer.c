 ...
 *
 * @param timer:    The target timer used in determining the elapsed time
 ...
 *
 * @param timer:    The target timer used in determining the percent progress elapsed
 * @return the percentage of time which has elapsed since the timer was started.
 */
double SWTimer_percentElapsed(SWTimer* timer)
{
    if (timer->cyclesToWait == 0) {
        return 1.0;
    }

    uint64_t elapsedCycles = SWTimer_elapsedCycles(timer);

    double result = (double) elapsedCycles / (double) timer->cyclesToWait;
